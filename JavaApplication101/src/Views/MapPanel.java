...
import javax.swing.*;
 ...
 public class MapPanel extends JPanel{
    ...
    /** 
     * Crea el panel de visualizacion del mapa.     
     */
    public MapPanel(){        
    }
    ...
    /** 
     * Pinta sobre el panel los elementos.     
     */
    public void setElements(Graphics g){
        int x, y, a, b, c, d;
        for(int i=0; i < dat.size(); ++i){
            a = Integer.parseInt(dat.get(i)[1]);            
            b = Integer.parseInt(dat.get(i)[2]);
            c = Integer.parseInt(dat.get(i)[3]);
            d = Integer.parseInt(dat.get(i)[4]);
            
            for(x=0; x < a-5; ++x){
                for(y=0; y < b-5; ++y){
                    Color color = getColor(dat.get(i)[0]);                  
                    cuadradito(g, xMin+c+x, yMin+d+y, color);                    
                }
            }        
            g.setColor(Color.black);            
            g.drawString(dat.get(i)[0], xMin+c+(a-dat.get(i)[0].length())/4, yMin+d+(b/2));
          
        }        
    }
    
    /** 
     * Pinta sobre el panel un cuadrado de 5x5 pixeles y del color establecido.
     * @param x Posicion en el eje de las X.
     * @param y Posicion en el eje de las Y.
     * @param color Color.
     */
    public void cuadradito(Graphics g, int x, int y, Color color){
        g.setColor(color);          
     ...
     * @return Retorna el color de ese elemento.
     */
    public Color getColor(String name){
        Color color;
        if(!cjtColor.containsKey(name)){
            float r = rand.nextFloat();
            float g = rand.nextFloat();
            float b = rand.nextFloat();
            color = new Color(r, g, b);
            cjtColor.put(name, color);
        }
        else{
            color = cjtColor.get(name);
        }
        return color;
    }
   ...
   /** 
     ...
     * @param window EConjunto de ventanas.
     */
    public void dibujar(int width, int length, int xMin, int yMin, int xMax, int yMax, List<String[]> dat, int[][] door, int[][] window){      
       this.xMin = xMin;
       this.yMin = yMin;
       this.xMax = xMax;
       this.yMax = yMax;
